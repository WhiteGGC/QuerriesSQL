...
DATEDIFF(DAY, lc.data_locacao, lc.data_devolucao) AS dias_aludado,
fm.titulo, fm.ano 
FROM cliente cl, locacao lc, dvd, filme fm
...
FROM estrela es, filme fm, filme_estrela fe
WHERE es.id = fe.estrelaid
	AND fm.id = fe.filmeid
	AND fm.ano = '2015'

/*3*/
SELECT fm.titulo, CONVERT(VARCHAR(10), dvd.data_fabricacao, 103),
CASE WHEN ((YEAR(GETDATE()) - fm.ano) > 6)
	THEN
		CAST((YEAR(GETDATE()) - fm.ano) AS VARCHAR(5)) + ' anos' 
	ELSE
		CAST((YEAR(GETDATE()) - fm.ano) AS VARCHAR(5))
END AS idade_filme
FROM filme fm, dvd
WHERE fm.id = dvd.filmeid
